...
begin
	using CSV, DataFrames, TimeSeries, DataStructures, Plots, PlutoUI,  Clustering, GraphPlot, Graphs, Multigraphs, BSON, TextAnalysis, URIs
	using IterTools: product
	using StatsBase: maximum, minimum, median, mean
	import Base, Statistics.mean
	import PyPlot, PlotlyBase, PlutoPlotly
	include("../influencecascade_platform.jl")
	include("../entropy.jl")
	include("../tool_helper.jl")
end;

# ╔═╡ f7e7e7aa-6a60-416a-a622-3df0a0b0b264
html"""<style>
	main {
	    max-width: 1200px;
	}
...
begin
	data = CSV.read("../../Data/BrandWatch/ExampleBWData.csv", DataFrame; header=7);
		...
		part_fun = partition_options[1]
		#Choose the way to define distinct action types: $(@bind action_fun Select(action_options))
		action_fun = action_options[3]
	
	md"""
	## Motivating Scenario
	#### Image you are a PSYOPS analyst who is tasked with countering adversary messaging of the local population on social media 
	#### Given a sample of tweets referencing news articles we aim to Identify and target counter-messaging at an audience whose social media activity influences the greatest proportion of the population.
	#### To achieve this, we must:
	##### Observe the information enviornment and meaningfully segment the population into audiences
	##### Given this sgmentation, identify a target audience which will maximize the effectiveness of our counter-messaging
	"""
end
...
begin
	md"""
	## Surveillance
	##### We begin open-ended exploration of the structure of the information enviornment as observed using transfer entropy 
	###### We can compare different methods of audience segmentation to identify particular target audiences
	\
	Select method for segmenting groups of actors in the information enviornment: $(@bind actor_fun Select(actor_options))

	###### Or test the sensitivity of the observed influence network to the connection strength
	"""
end
...
begin
	...
	md"""
	Connection Strength:   $(@bind cuttoff Slider(0:0.01:4, default=1.0, show_value=true))\
	"""
...
begin
	emb_meth = casc_emb_options[1]
	clust_meth = casc_clust_options[1]
	...
	md"""
	## Reconnaissance
	##### Having identified sufficient paramters to segment the information enviornment into meaningful audiences, we can now begin answer more specific questions about the dynamics of information and influence between our defined audiences.
	##### In order to most efficiently spread our counter-messaging, we want to target an audience who's influence is largest. Therefore, we want to identify a target audience who's twitter posts influence the maximum number of other audiences.
	###### Clustering based on the number of influenced nodes at each layer of an actor's influence cascade allows us to idenitify a set of actor groups which produce outsized influence and may be important populations to target with counter messaging.
...
begin
	if clust_meth==dbscan_clusterer
		md"""
		Choose clustering parameters:
	
		Cluster sensitivity: $(@bind ε Slider(0.0001:0.0001:1, show_value=true, default=0.005))\
		Minimum examples per clusters: $(@bind minpts Slider(1:20, show_value=true, default=5))
		"""
	end
end
...
begin
	idxs = vcat([0, length.(influence_cascades)]...)
	partition_map = Dict()
	for i in 2:length(idxs)
		for j in 1:idxs[i]
			partition_map[j+sum(idxs[1:(i)-1])] = i-1
		end
	end
	
	if clust_meth==dbscan_clusterer
		cc = CascadeClusterer(x->emb_meth(x;cuttoff=cuttoff), x->clust_meth(x; eps=ε, minpts=minpts))
		clusters = observe(all_ics[length.(nodes.(all_ics)).>0], cc)
		clusters
	end
	md"""
	We find $(length(clusters)) clusters.\
	They have sizes of $(string.(length.(clusters)) .* ", ") respectively.
	
	Choose the clusters you want to compare:\
	$(@bind c1 Select(1:length(clusters)))
	"""
end
...
md"""
##### Cluster $(c1)
Contains $(length(clusters[c1])) cascades
"""

# ╔═╡ 0189f430-d568-45e7-863f-27b0f82b44b9
#=
begin
	plts1 = []
	for c in clusters[c1]
		p, j = partition_map[c], c%idxs[1+partition_map[c]]+1
		push!(plts1, string("Cascade for actor '",df.actor[c], "'") => plot_cascade(
		influence_cascades[p][j],
		unique(df.action),
		length(unique(df.action)),
		cuttoff,
		range(colorant"blue", colorant"red", length(unique(df.action)))))
	end
	plts1
end
=#
...
begin
	...
	for c in clusters[c1]
		p, j = partition_map[c], c%idxs[1+partition_map[c]]+1
		push!(plts1, string("Cascade for actor '",df.actor[c], "'") => PlutoPlotly.plot(plot_cascade_sankey(
		influence_cascades[p][j],
		unique(df.action),
		cuttoff)...))
	end
...
# ╔═╡ 87c68416-607a-4d8c-bbf0-49fd785a1aad
md"""
Choose the second cluster to compare\
$(@bind c2 Select((1:length(clusters))[Not(c1)]))
##### Cluster $(c2)
Contains $(length(clusters[c2])) cascades
"""
...
begin
	...
	for c in clusters[c2]
		p, j = partition_map[c], c%idxs[1+partition_map[c]]+1
		push!(plts2, string("Cascade for actor '",df.actor[c], "'") => plot_cascade(
		influence_cascades[p][j],
		unique(df.action),
		length(unique(df.action)),
		cuttoff,
...
begin
	...
	for c in clusters[c2]
		p, j = partition_map[c], c%idxs[1+partition_map[c]]+1
		push!(plts2, string("Cascade for actor '",df.actor[c], "'") => PlutoPlotly.plot(plot_cascade_sankey(
		influence_cascades[p][j],
		unique(df.action),
		cuttoff)...))
	end
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "bfd35594ad1d3e7ab1cc826bed099a9f8b7cf0b8"
