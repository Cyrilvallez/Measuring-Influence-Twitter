...
all_actors = [retweet_count(by_partition=true, min_tweets=3, actor_number=600, aggregate_size=size) for size in aggregate_sizes]

all_agents = [PreProcessingAgents(partitions, actions, actors) for actors in all_actors]

# Create the experiment names
experiment_names = "JDD_aggregates_study/RT/" .* ["aggregate_size_$size" for size in aggregate_sizes]
